...
//Functions.
void initialize_car(Car*);			//Initializes the values of the car instance created.
void initialize_driver(Driver*);		//Initializes the values of the driver instance created.
int read_speed_sensor(Car*, Driver*);		//Reads the speed of the car from the instance variable created.
float rare_event(float p_event);		//Takes prob of an event happening and generates a random float value for brake, clutch if event ocurs.
...
int main()
{
	...
	initialize_car(simulationCar);
	initialize_driver(simulationDriver);
...
///////HELPER FUNCTIONS
void initialize_car(Car* simulationCar)
{
...
void initialize_driver(Driver* simulationDriver)
{
...
int read_speed_sensor(Car* simulationCar, Driver* simulationDriver)
{
	...
	float sample_brake= rare_event(0.05);		//15% chance of braking.
	float sample_clutch= rare_event(0.02);		//20% chance of clutch.
...
float rare_event(float p_event)
{
	float val_gen = (float) rand() / (float)RAND_MAX;
	return val_gen < p_event ?  (0.5f + ((float)rand() / RAND_MAX) * 0.5f) : 0.0f;
}
