...
bool is_preemption_active() {
    return emergency_preemption_active;
}

int calculate_normal_light_state() {
    int new_state;
    if (cycle_timer < 30) {
    ...
    log_light_state("Normal Cycle", new_state);
    return new_state;
}

int step_control_logic() {
    if (is_preemption_active()) {
        log_light_state("EMERGENCY PREEMPTION", STATE_RED);
        return STATE_RED;
    } else {
        return calculate_normal_light_state();
    }
}
...
int main() {
    ...
    printf("--- Traffic Light Controller Simulation (Multi Function) ---\n");
