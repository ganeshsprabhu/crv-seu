...
void read_mine_sensors() {
    methane_level_ppm = 400 + (rand() % 200);
...
bool is_methane_level_high() {
    return methane_level_ppm > METHANE_THRESHOLD;
}

int calculate_normal_fan_speed() {
    if (is_work_shift_active) {
        log_fan_speed("Work Shift Active", FAN_SPEED_NORMAL);
        return FAN_SPEED_NORMAL;
    } else {
        log_fan_speed("Off-Shift Idle", FAN_SPEED_IDLE);
        return FAN_SPEED_IDLE;
    }
}

int step_control_logic() {
    if (is_methane_level_high()) {
        log_fan_speed("HIGH METHANE - PURGE", FAN_SPEED_MAX);
        return FAN_SPEED_MAX;
    } else {
        return calculate_normal_fan_speed();
    }
}
...
int main() {
    ...
    printf("--- Mining Ventilation Control Simulation (Multi Function) ---\n");
